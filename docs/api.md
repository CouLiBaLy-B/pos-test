...
curl -X POST http://localhost:8080/api/v1/chat \
  ...
  -d '{
    ...
    "messages": [
      ...
      {"role": "assistant", "content": "Le dépôt orchestre Claude Code, LiteLLM et vLLM."},
...
- `ASSISTANT_UPSTREAM_BASE_URL` : endpoint Anthropic-compatible cible, défaut `http://litellm:4000` en Docker
- `ASSISTANT_AUTH_TOKEN` : token envoyé à l'upstream
